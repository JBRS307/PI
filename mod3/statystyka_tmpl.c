...
int my_round(double x){ //piszę własnego rounda, ponieważ z jakiegoś powodu wbudowany nie chce działać
    int y = x;
    double check = x-y;
    return (check < 0.5 ? y : y+1);
}

// Oblicza wartość średniej arytmetycznej oraz wariancji próby losowej
void aver_varian(const double arr[], size_t n, double *arith_average, double *variance){
    *arith_average = 0;
    *variance = 0;
    for(int i = 0; i < n; i++){
        *arith_average += arr[i];
    ...
    *arith_average /= n;
    ...
    for(int i = 0; i < n; i++){
        *variance += (arr[i] - *arith_average)*(arr[i] - *arith_average);
    }
    *variance /= n;
}
...
// 1 - sukces; 0 - porażka
void bernoulli_gen(int arr[], size_t n, double p){
    unsigned int cond = (unsigned int)RAND_MAX+1;
    for(unsigned int i = 0; i < n; i++){
        arr[i] = (rand() < p*(cond) ? 1 : 0);
    }
}
...
void print_histogram(const double arr[], size_t n, int x_start, double y_scale, char mark){
    int x = x_start;
    int length;
    for(int i = 0; i < n; i++, x++){
        printf("%2d ", x);
        printf("|");
        length = my_round(arr[i]/y_scale);
        for(int j = 0; j < length; j++) printf("%c", mark);
        printf(" %.3f\n", arr[i]);
    }
}
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n){
    ...
    for(int i = 0; i < n; i++){
        win = rand()%3+1;
        choice = rand()%3+1;
        if(win == choice) ++*p_nonswitch_wins;
        else ++*p_switch_wins;
    }
}
