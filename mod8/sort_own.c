...
int compar_date(const void *p1, const void *p2){
    const Date *a = (const Date*)p1;
    const Date *b = (const Date*)p2;
...
int compar_food(const void *p1, const void *p2){
    const Food *a = (const Food*)p1;
    const Food *b = (const Food*)p2;

    int cmp_art = strcmp(a->art, b->art);

    if(cmp_art == 0){
        float cmp_price = a->price - b->price;
        if(cmp_price == 0){
            return compar_date(&a->valid_date, &b->valid_date);
        }
        return cmp_price < 0 ? -1 : 1;
    }
    return cmp_art;
}

// size to rozmiar elementu
void *bsearch2(const void *key, void *base, size_t nitems, size_t size, ComparFp compar, cbool *result){
    char *curr = (char*)base; // to działa, char* przeskakuje o 1 bajt

    while(curr != base+(nitems*size)){
        int cmp = compar((void*)curr, key);
        if(cmp == 0){
            *result = true;
            return (void*)curr;
        }else if(cmp > 0){
...
void print_art(Food *p, size_t n, char *art){
    for(size_t i = 0; i < n; i++){
        if(!strcmp(p[i].art, art)){
            printf("%.2f %.2f ", p[i].price, p[i].amount);
...
Food* add_record(Food *tab, size_t tab_size, int *np, ComparFp compar, Food *new){
    cbool result;
    Food *addr = (Food*)bsearch2((const void*)new, (void*)tab, (size_t)*np, sizeof(Food), compar, &result);

    if(result){ // znaleziono taki element w tablicy;
        addr->amount += new->amount;
    }else{ // elementu key nie znaleziono
        if((size_t)(*np+1) > tab_size)
            return NULL;

        if(addr == tab+(*np)){
            memcpy(addr, new, sizeof(Food));
        }else{
            size_t to_move = *np - (addr-tab);
            memmove(addr+1, addr, sizeof(Food)*to_move);
            memcpy(addr, new, sizeof(Food));
        }
        (*np)++;
    }
...
int read_stream(Food *tab, size_t size, int no, FILE *stream){
    ...
    for(int i = 0; i < no; i++){
        Food f;
        fscanf(stream, "%s %f %f %d %d %d", f.art, &f.price, &f.amount, &f.valid_date.day, &f.valid_date.month, &f.valid_date.year);
        add_record(tab, size, &elems, compar_food, &f);
    }
    return elems;
}
...
// najmniejszy koszt obliczeniowy jest wtedy, kiedy zrobię to liniowo, bez sortowania
// zwłaszcza, że to jest posortowane

// funkcje do policzenia różnicy między datami w dniach
...
float value(Food *food_tab, size_t n, Date curr_date, int delta){
    float res = 0;
    for(size_t i = 0; i < n; i++){
        if(day_diff(curr_date, food_tab[i].valid_date) == delta)
            res += food_tab[i].price*food_tab[i].amount;
    }
    return res;
}
